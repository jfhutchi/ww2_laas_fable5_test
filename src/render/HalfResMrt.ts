/**
 * One half-res MRT quad pass for screen-space layers that don't need full
 * resolution (GTAO; later cloud march / bounce gather can join). Replaces
 * N separate half-res passes with a single raster — one encoder, shared
 * depth fetches, no per-pass RT round-trips. (LAAS port.)
 *
 ...
 *   quad.render with our RT bound triggers the compile. Wrapping the mrt()
 *   node (e.g. .context()) hides `isOutputStructNode` and breaks the build.
 ...
 * - `resolution` is a live vec2 uniform of the true half-res dimensions;
 *   the orchestrator aliases it into pre-created uniforms (e.g. the GTAO
 *   noise-tiling resolution) via `preMade.value = halfPass.resolution.value`
 *   — setSize mutates the shared Vector2 in place, so both stay current.
 */
...
export class HalfResMrtNode extends TempNode {
  /** half-res dimensions, live-updated in place — GTAO noise tiling reads this */
